 ...
 */
const setupMainPackageWatcher = (viteDevServer, starterServer) => {
    ...
    {
        const protocol = `http${starterServer.config.server.https ? 's' : ''}:`;
        const host = starterServer.config.server.host || 'localhost';
        const port = starterServer.config.server.port; // Vite searches for and occupies the first free port: 3000, 3001, 3002 and so on
        const path = '/';
        process.env.VITE_STARTER_SERVER_URL = `${protocol}//${host}:${port}${path}`;
    }
 ...
 * @returns {Promise<import('vite').RollupOutput | Array<import('vite').RollupOutput> | import('vite').RollupWatcher>}
 */
const setupPreloadPackageWatcher = (viteDevServer, starterServer) => {
    return getWatcher({
